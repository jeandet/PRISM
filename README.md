...
The same application code builds for the web with [Emscripten](https://emscripten.org) 6.0.10+. There is no reflection there (upstream clang lacks P2996), so models need a `view()` method. **Live demos:** <https://jeandet.github.io/PRISM/> (built and deployed by `.github/workflows/pages.yml`; `scripts/build_web_site.sh` assembles the same site locally).
